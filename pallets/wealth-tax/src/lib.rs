...
use frame_support::{
    ...
    traits::{
        ...
        LockableCurrency, ReservableCurrency, SignedImbalance, TryDrop, WithdrawReason,
        WithdrawReasons,
    },
...
use sp_core::{Hasher, H512};
use sp_runtime::{
    traits::{
        AtLeast32Bit, AtLeast32BitUnsigned, CheckedSub, MaybeSerializeDeserialize, Member,
        Saturating, Scale, Verify, Zero,
    },
    AnySignature, DispatchError, Perbill, RuntimeDebug,
};
...
impl<BN: AtLeast32Bit + Copy, BA: AtLeast32BitUnsigned + Copy> AgedUTXO<BA, BN> {
...
impl<BN: AtLeast32BitUnsigned + Copy, BA: AtLeast32BitUnsigned + Copy> Imbalance<BA>
    for AgedUTXO<BA, BN>
{
    ...
    fn merge(self, other: Self) -> Self {
        let rated_value = self
            .rate
            .mul(self.value)
            .saturating_add(other.rate.mul(other.value));
        let (higher, lower) = (
            self.clone().max(other.clone()),
            self.clone().min(other.clone()),
        );
        let value_ratio = Perbill::from_rational_approximation(lower.value, higher.value);
...
impl<BN: AtLeast32BitUnsigned + Copy, BA: AtLeast32BitUnsigned + Copy> TryDrop
    for AgedUTXO<BA, BN>
{
...
impl<T: Trait> Module<T> {
        ...
        if let Some(lock_id) = maybe_id {
            ...
            match (
                ...
                (Some(mut lock), Some(mut utxo)) => {
                    ...
                    match lock.peek().cmp(&utxo.peek()) {
                        ...
                        std::cmp::Ordering::Greater => {
                            ...
                            if let Ok(difference_imbalance) = Self::withdraw(
                                who,
                                difference,
                                WithdrawReasons::all(),
                ...
                (Some(lock), None) => {
                    ...
                    if let Ok(mut new_utxo) = Self::withdraw(
                        who,
                        lock.peek(),
                        WithdrawReasons::all(),
                        ExistenceRequirement::KeepAlive,
            ...
            let all_locks: Vec<LockIdentifier> = UTXOStore::<T>::iter_prefix(who)
                .filter_map(|(id, _)| Self::utxo_id_to_lock_id(id))
                .collect();
    ...
    fn new_utxo(
        ...
        id: Vec<u8>,
        permissions: WithdrawReasons,
    ) -> AgedUTXO<T::Balance, T::BlockNumber> {
        AgedUTXO {
            value: value,
...
pub trait Commitment<AccountId> {
    fn inc_commit(who: AccountId) -> Result<(), DispatchError>;
    fn consume_commit(who: AccountId) -> Result<(), ()>;
}
pub trait CustomLockableCurrency<AccountId, Balance, IdType, Imbalance> {
    ...
    fn custom_burn(who: &AccountId, value: Balance, id: IdType) -> Result<(), ()>;
}

impl<T: Trait> Commitment<T::AccountId> for Module<T> {
	...
	fn inc_commit(who: T::AccountId) -> Result<(), DispatchError> {
        ...
        Self::custom_resolve(
            ...
            imbalance,
            UTXOIdType::UTXO(id),
            WithdrawReasons::except(WithdrawReason::Transfer.into()),
        );
    ...
    > for Module<T>
{
    ...
    fn custom_resolve(
        who: &T::AccountId,
        imbalance: AgedUTXO<T::Balance, T::BlockNumber>,
        id: UTXOIdType<Vec<u8>>,
        permissions: WithdrawReasons,
    ) {
        //we get the existing balance if any exists
...
impl<T: Trait> Currency<T::AccountId> for Module<T> {
    ...
    fn total_balance(who: &T::AccountId) -> Self::Balance {
        ...
        if let Some(free_utxos) = Self::filter_utxos_for(who, WithdrawReasons::none(), None) {
    ...
    fn free_balance(who: &T::AccountId) -> Self::Balance {
        let mut out_balance = Self::Balance::zero();
        if let Some(free_utxos) = Self::filter_utxos_for(who, WithdrawReasons::all(), None) {
            for (_id, mut utxo) in free_utxos {
                utxo.update(<system::Module<T>>::block_number());
                out_balance = out_balance.saturating_add(utxo.value);
            }
        ...
        if let Some(inputs) = maybe_inputs {
            ...
            for input in inputs {
                ...
                if let Some(surplus) = merged.peek().checked_sub(&value) {
                    let utxo_change = Self::new_utxo(surplus, input.0, permissions);
    ...
    fn deposit_creating(who: &T::AccountId, value: Self::Balance) -> Self::PositiveImbalance {
        let new = Self::new_utxo(value, Vec::<u8>::default(), WithdrawReasons::all());
        if let Some(mut recipient_utxo) =
            UTXOStore::<T>::take(who, UTXOIdType::UTXO(Vec::default()))
        {
    ...
    fn make_free_balance_be(
        ...
        if let Some(slashable_surplus) = balance.checked_sub(&current_balance) {
            if let Ok(withdrawal) = Self::withdraw(
                who,
                slashable_surplus,
                WithdrawReasons::all(),
...
impl<T: Trait> LockableCurrency<T::AccountId> for Module<T> {
    ...
    fn set_lock(
        ...
        UTXOStore::<T>::insert(
            who,
            UTXOIdType::Lock(id.into()),
            Self::new_utxo(amount, id.into(), reasons),
        );
...
impl<T: Trait> pallet_onboard::traits::PowVerifier<T::AccountId, H512> for Module<T> {
    ...
    fn verify(author: T::AccountId, pow: H512) -> bool {
        ...
        if let Some((committed, rewarded)) = CommitmentSecrets::<T>::get(secret_hash) {
            if rewarded == author {
                //check this first to gate potential commitment consumption
